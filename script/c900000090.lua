...
function c900000090.initial_effect(c)
	...
	e1:SetTarget(c900000090.target)
	e1:SetCondition(c900000090.spcon)
	e1:SetOperation(c900000090.activate)
	c:RegisterEffect(e1)
end
...
function c900000090.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c900000090.filter,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil,e,tp) end
...
function c900000090.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1 = Duel.SelectMatchingCard(tp, c900000090.filter1, tp, LOCATION_DECK + LOCATION_HAND + LOCATION_GRAVE, 0, 1, 1, nil, e,
		tp)
	Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SPSUMMON)
	local g2 = Duel.SelectMatchingCard(tp, c900000090.filter2, tp, LOCATION_DECK + LOCATION_HAND + LOCATION_GRAVE, 0, 1, 1, nil, e,
		tp)
	g1:Merge(g2)
	if #g1 == 2 then
...
function c900000090.filter1(c,e,tp)
	return c:IsCode(900000089)
end

function c900000090.filter2(c,e,tp)
